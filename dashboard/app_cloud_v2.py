...
with st.sidebar:
    ...
    algorithm = st.selectbox("Algoritma Load Balancer", ["round_robin", "random", "least_connection"],
...
if not run:
    ...
    # ── Stability alert ───────────────────────────────────────────
    if rho >= 1:
        st.markdown(f'<div class="alert-critical">🔴 <b>SISTEM TIDAK STABIL</b> — ρ = {rho:.2f} ≥ 1. Antrian akan terus membesar tanpa batas. Tambah worker atau kurangi λ.</div>', unsafe_allow_html=True)
    elif rho >= 0.8:
        st.markdown(f'<div class="alert-warn">⚠️ <b>Mendekati kritis</b> — ρ = {rho:.2f}. Sistem mulai jenuh, wait time meningkat signifikan.</div>', unsafe_allow_html=True)
    else:
        st.markdown(f'<div class="alert-stable">✅ <b>Sistem stabil</b> — ρ = {rho:.2f}. Kapasitas server mencukupi untuk menangani beban.</div>', unsafe_allow_html=True)
    ...
    metrics = [
        ...
        (c4, "Avg Wait Time", f"{avg_wait:.3f}s", "Wq rata-rata", "warn" if avg_wait > 1 else "good"),
    ...
    with col_r:
        ...
        fig2.add_trace(go.Scatter(x=x_range, y=y_exp, mode="lines",
            line=dict(color="#ef9f27", width=2, dash="dot"), name="Exp(μ) teoritis"))
    ...
    with col_l2:
        ...
        colors = ["#4caf50" if q == 0 else "#ff9800" if q <= 2 else "#f44336"
    ...
    util_df = df.groupby("interval").apply(
        lambda g: min(len(g) / (5 * lambda_rate), 1.5)
    ...
    for algo, res in all_results.items():
        if res:
            ...
            fig6.add_trace(go.Box(
                y=d["wait_time"], name=algo.replace("_", " ").title(),
                marker_color=algo_colors.get(algo.replace("_", " ").title(), "#64b5f6"),
                boxmean=True
            ))
