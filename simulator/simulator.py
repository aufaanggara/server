...
def run_one_algorithm(algorithm: str) -> list:
    ...
    while time.time() - start_time < DURATION:
        ...
        t = threading.Thread(target=send_request, args=(request_id, algorithm, results))
        t.start()
        threads.append(t)
        request_id += 1
...
def print_comparison(all_results: dict):
    ...
    print("=" * 60)
    print(
        f"{'Algoritma':<20} {'Total Req':<12} {'Avg Service':<14} {'Avg Total':<12} {'Throughput'}"
    )
    print("-" * 60)
    ...
    for algo, results in all_results.items():
        ...
        print(
            f"{algo:<20} {len(results):<12} {avg_service:<14.4f} {avg_total:<12.4f} {throughput:.2f} req/s"
        )
