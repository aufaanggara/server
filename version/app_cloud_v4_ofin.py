...
#     algo_label = {"round_robin": "Round Robin", "random": "Random", "least_connection": "Least Connection"}[algorithm]
#     rho = lambda_rate / (num_workers * mu)
#     rho_color = "#4caf50" if rho < 0.7 else "#ff9800" if rho < 1 else "#f44336"
#     initial_paused_js = "true" if paused else "false"
...
#     #formulaBar .algoDesc {{ color:#5c8a9e; margin-left:auto; font-family:'Segoe UI',sans-serif; font-size:10px; font-style:italic; }}
...
#     #pauseOverlay span {{
#       font-size:28px; font-weight:700; color:#64b5f6;
...
#     <div class="info-value" style="font-size:14px;color:#ce93d8">{algo_label}</div>
...
#     <div class="info-sub">{"✅ stabil" if rho < 0.7 else "⚠️ mendekati kritis" if rho < 1 else "🔴 kritis!"}</div>
#   </div>
# </div>
...
#   <span style="color:#1e3a5f">|</span>
#   <span>W = Wq + 1/μ</span>
#   <span class="algoDesc">{{"round_robin":"Round Robin — bergiliran ke setiap worker","random":"Random — worker dipilih secara acak","least_connection":"Least Connection — worker dengan antrian terpendek"}}["{algo_js}"]</span>
# </div>
...
#   // Listen for postMessage from parent (Streamlit) — future-proof
#   window.addEventListener("message", e => {{
#     if(e.data === "pause") {{ paused=true;  document.getElementById("pauseOverlay").classList.add("visible"); }}
#     if(e.data === "play")  {{ paused=false; document.getElementById("pauseOverlay").classList.remove("visible"); }}
#     if(e.data === "reset") {{
#       packets=[]; trails=[]; workers=[]; reqId=0; nextSpawn=0;
#       initWorkers();
...
# # ─────────────────────────────────────────────────────────────────
# # SIDEBAR — tempel ini di blok `with st.sidebar:` kamu
# # (ganti seluruh blok sidebar lama dengan ini)
# # ─────────────────────────────────────────────────────────────────
...
#     # ── Parameter ──────────────────────────────────────────────────
#     st.markdown(
#         \'<div class="section-header">Parameter Stokastik</div>\',
#         unsafe_allow_html=True,
#     )
...
#                              format="%.1fx",
#                              help="Kecepatan simulasi animasi")
...
#     # ── Konfigurasi ────────────────────────────────────────────────
#     st.markdown(
#         \'<div class="section-header">Konfigurasi Simulasi</div>\',
#         unsafe_allow_html=True,
#     )
...
#     # ── Kontrol Animasi ────────────────────────────────────────────
#     st.markdown(
#         \'<div class="section-header">Kontrol Animasi</div>\',
#         unsafe_allow_html=True,
#     )
...
#     stress_label = "⏹ Stop Stress" if st.session_state.sim_stress else "📈 Stress Test"
#     if st.button(stress_label, use_container_width=True):
#         st.session_state.sim_stress = not st.session_state.sim_stress
#         if st.session_state.sim_stress:
#             # Paksa λ ke max saat stress aktif
#             lambda_rate = 5.0
#         st.rerun()

#     if st.session_state.sim_stress:
#         st.markdown(
#             \'<div style="background:rgba(230,81,0,.15);border:1px solid #e65100;border-radius:6px;padding:6px 10px;font-size:11px;color:#ffcc80;margin-top:4px">\'
#             \'📈 Stress aktif — λ dipaksa ke 5.0 req/s</div>\',
#             unsafe_allow_html=True,
#         )
...
#     # ── Info model ─────────────────────────────────────────────────
#     st.markdown(
#         \'<div class="section-header">Tentang Model</div>\',
#         unsafe_allow_html=True,
#     )
#     st.markdown(
#         """
...
# # ─────────────────────────────────────────────────────────────────
# # Cara pakai get_animation_html() yang baru:
# #
...
# #   )
# #
# # Key `st.session_state.sim_reset_key` dipasang sebagai key di
# # st.components agar iframe di-remount saat Reset ditekan:
# #
# #   key=f"anim_{st.session_state.sim_reset_key}"
# #
# # Contoh lengkap di bagian animasi:
# #
# #   st.components.v1.html(
...
# #       height=650,
# #       scrolling=False,
# #       # key tidak didukung di st.components.v1.html,
# #       # tapi Streamlit akan re-render otomatis saat state berubah
# #   )
# # ─────────────────────────────────────────────────────────────────
...
#             "Request Selesai",
#             len(df),
#             f"{round(len(df)/len(df)*100)}% success",
#             "good",
#         ),
